...
webrtc.addEventListener('newUser', (event) => {
    const videoElement = document.createElement('video');
    videoElement.srcObject = event.detail.stream;
    videoElement.autoplay = true;
    videoElement.playsInline = true;
    videoElement.setAttribute('data-socket', event.detail.socketId); // Assign socketId to video element for reference
    document.getElementById('videoGrid').appendChild(videoElement);
});

webrtc.addEventListener('removeUser', (event) => {
    const videoElement = document.querySelector(`video[data-socket="${event.detail.socketId}"]`);
    if (videoElement) {
        videoElement.remove();
    }
});
